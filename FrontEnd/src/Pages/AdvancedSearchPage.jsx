...
export default function AdvancedSearchPage() {
    ...
    function filterDoctors() {
        if (filterSpecialization === "Tutti") {
            ...
            return allDoctors.filter((doctor) => {
                ...
                const nameAndSurname = `${doctor.first_name.toLowerCase()} ${doctor.last_name.toLowerCase()}`
                return nameAndSurname.includes(searchTextLower)
    ...
    return (
        <div className={`container-sm container-md container-lg container-xl container-xxl ${style.dev_container}`}>
            ...
            <form className={style.d_flex} onSubmit={(e) => e.preventDefault()}>
                <select
                    className={`form-control mx-2 ${style.margin_bottom}`}
                ...
                <select
                    className={`form-select mx-2 ${style.margin_bottom}`}
                    aria-label="Default select example"
                    onChange={handleFilterChange}
            ...
            <h4 className='fw-bold mt-2'>{t(filterSpecialization)}</h4><span>Risultati: {filterDoctors().length}</span>
            ...
            {filterDoctors().map((doctor, index) => (
                ...
                >
                    <img src="/doctor_icon.png" alt="ProfileImg" />
                    <p><strong>Nome: </strong><br /> {doctor.first_name}</p>
                    <p><strong>Cognome: </strong><br /> {doctor.last_name}</p>
                    <p className={style.px576_p}> <strong>Rating: </strong> <br />{ratings[doctor.doctor_id] ? starRating(ratings[doctor.doctor_id]) : 'N/A'}</p>
                    <p className={style.px768_p}><strong>Specializzazione: </strong><br /> {filterSpecialization === "Tutti" ? (t(doctor.specializations.split(",").map((spec) => t(spec.trim())).join(", ").substring(0, 20)) + "...") : t(filterSpecialization)}</p>
